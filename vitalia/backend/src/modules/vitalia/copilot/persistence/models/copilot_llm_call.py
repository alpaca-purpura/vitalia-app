...
:class:`luana_core_copilot.observability.persistence.models.llm_call_model.CopilotLlmCallModel`.
...
Engine table: ``copilot_llm_call`` (owned by luana-core-copilot engine migrations).
"""
...
class CopilotLLMCallVitalia(Base):
    """Vitalia brand schema-mirror of the ``copilot_llm_call`` engine table.
    ...
    __table_args__ = (
        Index("ix_copilot_llm_call_tenant_time", "tenant_id", "started_at"),
        Index("ix_copilot_llm_call_turn", "turn_id"),
    )

    def __repr__(self) -> str:
        """Return debug-friendly representation."""
        return (
            f"<CopilotLLMCallVitalia id={self.id} provider={self.provider}"
