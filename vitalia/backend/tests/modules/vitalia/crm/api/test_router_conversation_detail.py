"""Tests for GET /crm/conversations/{conv_id}.

Covers:
  - Slice 1: doctor requests conv detail → 404 (no live DB yet)
...
# ---------------------------------------------------------------------------
# Slice 1 behavior — 404 until real repo injected
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_get_conversation_detail_404_slice1(
    mock_clinic_ctx_doctor: MagicMock,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Doctor requests conversation detail → 404 in Slice 1 (no live DB).
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        resp = await client.get(
            f"/api/v1/vitalia/crm/conversations/{CONV_ID}",
            headers={
    ...
    # Slice 1: always 404 (conv_repo not yet wired)
    assert resp.status_code == 404


# ---------------------------------------------------------------------------
# 403 — marketing role cannot view PHI conversation detail
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_get_conversation_detail_403_marketing(
