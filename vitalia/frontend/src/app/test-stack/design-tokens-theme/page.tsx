// cap: platform.design-tokens-theme
// story-origin: vitalia-fase1-s1-TBD
/**
 * /test-stack/design-tokens-theme — Visual baseline page F1-S1
 * (vitalia-fase1-design-tokens-theme)
 *
 * Serves the ThemeToggle showcase fixture as a Next.js route.
 * Per F1-S0 lesson: test pages in e2e/__test-pages__/ need a Next.js route
 * wrapper otherwise Playwright 404s. Route lives here outside auth route groups.
 *
 * Public dev-only page — no Clerk auth, no PHI (HIPAA-lite no-phi-scope).
 * See: 03-arch.md § 2.9 + vitalia/.claude/rules/hipaa-lite.md (no-phi-scope)
 */
import ThemeToggleShowcasePage from "@/../e2e/__test-pages__/design-tokens-theme/theme-toggle-showcase";

export default ThemeToggleShowcasePage;
