...
// story-origin: TBD
/**
 * AdoptionStage — adoption funnel stage panel.
...
export type AdoptionStageProps = {
  period?: "7d" | "30d" | "90d";
  className?: string;
};

/**
 * AdoptionStage — stage panel rendered by StageDispatcher for `adoption` tab.
 */
export const AdoptionStage = forwardRef<HTMLElement, AdoptionStageProps>(
  ({ period = "30d", className }, ref) => {
    const { data, isLoading } = useStageDetail({ stage: "adoption", period });
    ...
    if (isLoading) {
      return (
        <section
          ref={ref}
          role="tabpanel"
          id="stage-panel-adoption"
          aria-labelledby="stage-tab-adoption"
          aria-busy={true}
    ...
    return (
      ...
      >
        ...
        <LucasStageRecommendationsCard stage="adoption" />
      </section>
    );
  },
);
AdoptionStage.displayName = "AdoptionStage";
