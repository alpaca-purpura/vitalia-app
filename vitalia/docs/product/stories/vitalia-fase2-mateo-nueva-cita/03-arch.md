...
| `core/@luana/ui-kit/src/**` (4 atoms PROMOTE) | **`/pm-vitalia` promotion gate** (NOT a builder) | n/a (kit owns) |
...
- **Lift candidate (NOT now):** el cómputo free/busy + condición de solape half-open podrían lift a `core/luana-core-scheduling` si otra marca lo necesita → `/pm-vitalia` candidate. Documentado, no se ejecuta en esta story.
...
- **Engine `appointments` table**: EXTEND vía migración brand idempotente (DDL al schema compartido). **NO se edita el modelo del engine ni código `core/luana-core-scheduling/src/`** — solo se añade un constraint a la tabla existente vía SQL de la marca (ver § Architecture Decisions D-A: engine-boundary). Esto NO requiere `/pm-vitalia` lift (no toca código del engine, no cambia su contrato de columnas; un constraint es una garantía de integridad sobre datos que la marca posee operacionalmente). Si el auditor objeta → escalar `/pm-vitalia` (documentado abajo).
...
3. **Engine-boundary del EXCLUDE (D-E.1):** el constraint vive en `vitalia_appointment_clinic_map` (brand-local) → NO toca `core/luana-core-scheduling/src/`. Si el auditor-backend lo considera engine-touch, escalar `/pm-vitalia`. Architect afirma que NO es lift (cero edición de código engine).
