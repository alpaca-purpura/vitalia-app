...
# chris-input.md · vitalia-payment-adapter-mvp
...
Story migrada al schema v2: `release=F3`, `cap_target=null`, `cap_change_type=new`. Legacy `outcome` + `phase` preservados durante deprecation gradual.

Cuando estés listo para refinar, llena 💭 Notas + 📎 Referencias arriba e invoca `/po-ux vitalia vitalia-payment-adapter-mvp` (o `/po` si es service story, o `/ux-agentico` si es agentic).
