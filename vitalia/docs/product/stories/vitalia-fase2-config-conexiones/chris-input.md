...
# chris-input.md · vitalia-fase2-config-conexiones
...
Story migrada al schema v2: `release=F2`, `cap_target=config.conexiones`, `cap_change_type=new`. Legacy `outcome` + `phase` preservados durante deprecation gradual.

Cuando estés listo para refinar, llena 💭 Notas + 📎 Referencias arriba e invoca `/po-ux vitalia vitalia-fase2-config-conexiones` (o `/po` si es service story, o `/ux-agentico` si es agentic).
