...
class TestOfferReadPortImpl:
    ...
    def test_impl_implements_port(self):
        ...
        luana_core_offer = pytest.importorskip(
            "luana_core_offer.application.services.offer_read_port_impl",
            reason="Story 5 luana-core-offer not yet lifted",
        )
        from luana_core_analytics_engine.domain.ports import OfferReadPort

        OfferReadPortImpl = luana_core_offer.OfferReadPortImpl  # noqa: N806
    ...
    def test_impl_does_not_import_offer_domain(self):
        ...
        luana_core_offer_svc = pytest.importorskip(
            "luana_core_offer.application.services.offer_read_port_impl",
            reason="Story 5 luana-core-offer not yet lifted",
        )
        source = inspect.getsource(luana_core_offer_svc)
        assert "from luana_core_offer.domain" not in source, (
