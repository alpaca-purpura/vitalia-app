...
class CampaignSyncPipeline:
    ...
    async def run_sync(
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            campaigns_count = self._repo.upsert_campaigns(tenant_id, campaigns)
            ...
            ads_count = self._repo.upsert_ads(tenant_id, ads)
            ...
            recs_count = self._repo.upsert_recommendations(tenant_id, all_recs)
            ...
            stale_camps = self._repo.soft_delete_stale(
                tenant_id,
                "meta",
                "ad_campaigns",
                campaign_ids,
            )
            stale_adsets = self._repo.soft_delete_stale(
                tenant_id,
                "meta",
                "ad_sets",
                adset_ids,
            )
            stale_ads = self._repo.soft_delete_stale(
                tenant_id,
                "meta",
                "ads",
                ad_ids,
