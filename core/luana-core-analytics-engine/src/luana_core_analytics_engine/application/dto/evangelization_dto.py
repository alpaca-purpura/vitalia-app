...
class EvangelizationHeaderKpisDTO(BaseModel):
    ...
    nps_score: float | None = None  # 0-10 scale (average), None if no responses
...
class NpsSummaryDTO(BaseModel):
    ...
    nps_score: float | None = None  # 0-10 average
    standard_nps: float | None = None  # -100 to +100 (for tooltip)
