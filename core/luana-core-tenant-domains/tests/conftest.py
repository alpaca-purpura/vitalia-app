"""Fixtures for tenant_domains module tests in luana-platform.

Lifted from AISALESHT backend/tests/modules/tenant_domains/conftest.py.
...
os.environ.setdefault("CLOUDFLARE_ZONE_ID", "ci-dummy-zone-id")
os.environ.setdefault("CLOUDFLARE_API_TOKEN", "ci-dummy-cf-token")
...
def db_engine():
    # Register IAM models first (tenant_domains FK → tenants)
    ...
    from luana_core_tenant_domains.infrastructure.models.tenant_domain_model import TenantDomainModel  # noqa: F401
...
def sample_domain_model(db, tenant_id):
    ...
    from luana_core_tenant_domains.infrastructure.models.tenant_domain_model import TenantDomainModel
