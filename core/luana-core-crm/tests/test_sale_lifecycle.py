...
class TestModuleDecoupling:
    ...
    def test_sales_module_does_not_import_crm(self):
        ...
        import luana_core_crm.application.services.sale_service as sale_mod

        source = inspect.getsource(sale_mod)
        assert "from luana_core_crm.application.services.lifecycle_service" not in source
        assert "from luana_core_crm.application.services.customer_service" not in source
