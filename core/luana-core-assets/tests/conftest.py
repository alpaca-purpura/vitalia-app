"""Fixtures for assets module tests in luana-platform.

Lifted from AISALESHT backend/tests/modules/assets/conftest.py.
...
# Stub products table for FK resolution (offer module not yet lifted — Story 4)
_stub_products = Table(
    "products",
    Base.metadata,
...
def seed_other_tenant(db, other_tenant_id):
    """Persist a second TenantModel row for isolation tests."""
    from luana_core_iam.infrastructure.models.tenant_model import TenantModel
