...
SHARED_OBS_DIR = Path(__file__).resolve().parents[2] / "luana-core-observability" / "src" / "luana_core_observability"
...
def test_shared_agent_observability_does_not_import_modules() -> None:
    ...
    for py_file in sorted(SHARED_OBS_DIR.rglob("*.py")):
        ...
        rel = py_file.relative_to(SHARED_OBS_DIR.parents[0])  # package-relative
