...
def tool_create_booking_link(state: dict[str, Any], db: Session) -> dict[str, Any]:
    ...
    if existing is not None:
        return {
            "status": "success",
            "url": _reconstruct_url_from_existing(
                state, existing.tracking_id, event_slug
            ),
...
def tool_get_available_slots(state: dict[str, Any], db: Session) -> dict[str, Any]:
    ...
    if not slots:
        return {
            ...
            "slots": [],
            "message": (
                "No hay horarios disponibles en los próximos días. ¿Te aviso cuando se abra un nuevo bloque?"
            ),
        }
