    ...
    serialised = [
        {
            ...
            "seats_remaining": (e.capacity - e.enrollment_count)
            if e.capacity
            else None,
        }
        for e in editions
...
def tool_create_enrollment(state: dict[str, Any], db: Session) -> dict[str, Any]:
    ...
    payload = EnrollmentCreate(
        ...
        conversation_id=_coerce_uuid(state["conversation_id"])
...
def tool_mark_enrollment_paid_manual(
    state: dict[str, Any], db: Session
) -> dict[str, Any]:
...
def tool_promote_waitlist_to_edition(
    state: dict[str, Any], db: Session
) -> dict[str, Any]:
