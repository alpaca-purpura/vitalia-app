...
def _make_lookup_result(
    ...
    return CampaignTaskLookupResult(
        ...
        task_id=TASK_ID,
        sent_at=sent_at
        or (dt.datetime.now(tz=dt.timezone.utc) - dt.timedelta(hours=1)),
    )
...
def test_settings_has_inbound_recognition_window_hours() -> None:
    ...
    assert field_info is not None, (
        "Settings debe tener CAMPAIGNS_INBOUND_RECOGNITION_WINDOW_HOURS"
    )
...
def test_campaigns_inbound_window_chat_module_reference() -> None:
    ...
    assert "inbound_campaign" in src, (
        "PR-8: chat.py debe contener el bloque de reconocimiento inbound de campaña"
    )
