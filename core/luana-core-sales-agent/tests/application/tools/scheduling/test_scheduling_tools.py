...
def test_create_booking_link_idempotent_within_window(
    populated_db, tenant_id, lead_id
) -> None:
...
def test_create_booking_link_missing_event_slug(
    populated_db, tenant_id, lead_id
) -> None:
    state = _build_state(tenant_id, lead_id)
...
def test_create_booking_link_invalid_expires_in_hours(
    populated_db, tenant_id, lead_id
) -> None:
...
def test_verify_booking_status_unknown(populated_db, tenant_id, lead_id) -> None:
    ...
    assert (
        ...
        else result.get("tracking_id") == "not-real"
    )
...
def test_always_available_tools_resolve_in_registry() -> None:
    ...
    for name in ALWAYS_AVAILABLE:
        assert name in TOOL_REGISTRY, (
            f"ALWAYS_AVAILABLE[{name}] missing from TOOL_REGISTRY"
        )
