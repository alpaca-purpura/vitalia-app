...
def test_provider_implements_protocol() -> None:
    ...
    assert isinstance(
        InternalSchedulerProvider.__init__, type(InternalSchedulerProvider.__init__)
    )
...
def test_create_booking_link_persists_link(db, tenant, lead) -> None:
    ...
    with patch(
        ...
        mock_lookup.return_value.get_verified_domain.return_value = (
            None  # fallback to DASHBOARD_DOMAIN
        )
    ...
    rows = (
        db.execute(select(BookingLink).where(BookingLink.token == out.tracking_id))
        .scalars()
        .all()
    )
    assert len(rows) == 1
...
def test_verify_booking_status_confirmed_when_appointment_scheduled(
    db, tenant, lead
) -> None:
