...
@pytest.fixture
def checkpoint(
    db, tenant_id: uuid.UUID, lead_id: uuid.UUID
) -> AgentStateCheckpointModel:
...
def test_update_status_by_tracking_promotes_entry(
    db, tenant_id, lead_id, checkpoint
) -> None:
    service = MeetingStateService(db)
    booking = _booking()
    service.append_link(tenant_id, lead_id, booking)
...
def test_mark_reminder_sent_unknown_kind_raises(
    db, tenant_id, lead_id, checkpoint
) -> None:
    service = MeetingStateService(db)
    booking = _booking()
