...
def test_resolve_customer_telegram_uses_telegram_capture_slug() -> None:
    ...
    customer, was_created, slug, channel_type, identity_type = (
        IdentityResolver.resolve_customer(
            identity_service,
            incoming,
            TENANT_ID,
        )
...
def test_resolve_customer_unknown_channel_falls_back_to_external_id() -> None:
    identity_service = MagicMock()
    identity_service.get_or_create_customer.return_value = (
        SimpleNamespace(id=CUSTOMER_ID),
        False,
    )
    incoming = IncomingMessage(
        user_id="x", text="t", channel_type="unknown_channel", metadata={}
    )
...
@pytest.mark.asyncio
async def test_enrich_instagram_profile_swallows_failure(
