...
def test_load_checkpoint_returns_none_when_tenant_missing() -> None:
    ...
    result = ConversationPipeline.load_checkpoint(
        MagicMock(), state_repo, None, LEAD_ID
    )
...
async def test_handle_human_mode_returns_false_when_no_checkpoint() -> None:
    ...
    incoming = IncomingMessage(
        user_id="u", text="t", channel_type="telegram", metadata={}
    )
...
@pytest.mark.asyncio
async def test_handle_human_mode_increments_unread_and_emits_ws(
...
def test_determine_session_state_no_history_returns_defaults() -> None:
    ...
    result = ConversationPipeline.determine_session_state(
        audit_repo, SimpleNamespace(id=LEAD_ID)
    )
    assert result == {
        "session_active": True,
        "last_intent": None,
...
def test_build_user_profile_no_user_returns_empty() -> None:
    user = SimpleNamespace(
        profile_data=None, custom_system_instruction=None, style_profile=None
    )
...
async def test_sanitize_text_returns_sanitized(monkeypatch: pytest.MonkeyPatch) -> None:
    safety_stub = SimpleNamespace(
        sanitize_content=AsyncMock(return_value=("clean text", True))
    )
    monkeypatch.setattr(
...
@pytest.mark.asyncio
async def test_sanitize_text_returns_original_on_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def _factory() -> object:
        msg = "safety down"
...
@pytest.mark.asyncio
async def test_deliver_response_logs_sends_and_emits(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    safety_stub = SimpleNamespace(
        sanitize_content=AsyncMock(return_value=("respuesta", False))
    )
    monkeypatch.setattr(
        "luana_core_sales_agent.infrastructure.external.safety_service.SafetyLayerService",
        lambda: safety_stub,
    ...
    result = {
        "messages": [{"role": "user"}, {"role": "assistant", "content": "respuesta"}]
    }
