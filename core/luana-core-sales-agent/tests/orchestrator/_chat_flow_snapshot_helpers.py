...
def install_chat_module_patches(  # noqa: PLR0915 — orchestrator has many touchpoints
    ...
    monkeypatch.setattr(
        ...
        staticmethod(
            lambda event, session=None: _capture_publish(event, session=session)
        ),
    )
        ...
        capture.customer_traits_updates.append(
            {
                ...
                "metadata_keys": sorted(incoming.metadata.keys())
    ...
    monkeypatch.setattr(
        ...
        staticmethod(
            lambda _text, existing_signals, tenant_id: (
                None,
                0.0,
                list(existing_signals),
            )
        ),
    )
        ...
        capture.agent_invocations.append(
            {
                ...
                "callbacks_kind": _summarize_callbacks(
                    (config or {}).get("callbacks", [])
                ),
            },
        )
