...
def test_track_message_received_emits_track_event(
...
def test_track_message_received_omits_message_id_when_missing(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    journey_repo = MagicMock()
    monkeypatch.setattr(
    ...
    customer = SimpleNamespace(id=CUSTOMER_ID)
    incoming = IncomingMessage(
        user_id="u", text="t", channel_type="whatsapp", metadata={}
    )
...
def test_track_message_received_swallows_exception(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_publish_lead_captured_publishes_event(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        EventBusAdapter, "_is_outbox_enabled", staticmethod(lambda *_: False)
    )
...
@pytest.mark.asyncio
async def test_emit_assistant_message_sends_ws_payload(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    captured: list[tuple] = []
    ...
    await AuditEmitter.emit_assistant_message(
        TENANT_ID, user, "respuesta del bot", result
    )
...
@pytest.mark.asyncio
async def test_emit_assistant_message_truncates_to_200_chars(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    captured: list[dict] = []
...
@pytest.mark.asyncio
async def test_emit_assistant_message_swallows_ws_failure(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
@pytest.mark.asyncio
async def test_emit_human_mode_message_sends_user_role(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    captured: list[tuple] = []
