...
class TestBackwardsCompat:
    ...
    async def test_old_brand_call_still_dispatches(
    ...
    async def test_offer_id_deprecated_alias_still_works(
        self, mock_arq_pool: MagicMock
    ) -> None:
    ...
    async def test_old_scope_full_mode_initial_still_works(
        self, mock_arq_pool: MagicMock
    ) -> None:
    ...
    async def test_old_scope_visuals_brand_still_works(
        self, mock_arq_pool: MagicMock
    ) -> None:
...
class TestNewReturnShape:
    ...
    async def test_dispatched_response_has_target_label_es(
        self, mock_arq_pool: MagicMock
    ) -> None:
        result = await extract_from_url.ainvoke(
    ...
    async def test_dispatched_response_has_source_kind_url(
        self, mock_arq_pool: MagicMock
    ) -> None:
        result = await extract_from_url.ainvoke(
            {"module": "brand", "url": "https://example.com"},
    ...
    async def test_dispatched_response_has_source_ref(
        self, mock_arq_pool: MagicMock
    ) -> None:
        url = "https://example.com"
    ...
    async def test_dispatched_response_has_eta_seconds(
        self, mock_arq_pool: MagicMock
    ) -> None:
        result = await extract_from_url.ainvoke(
            {"module": "brand", "url": "https://example.com"},
    ...
    async def test_message_field_kept_for_backwards_compat(
        self, mock_arq_pool: MagicMock
    ) -> None:
    ...
    async def test_section_returned_when_provided(
        self, mock_arq_pool: MagicMock
    ) -> None:
        result = await extract_from_url.ainvoke(
            {
    ...
    async def test_target_label_es_for_section_scope(
        self, mock_arq_pool: MagicMock
    ) -> None:
        ...
        assert (
            "identidad" in label.lower()
            or "identity" in label.lower()
            or "brand" in label.lower()
        )
...
class TestNewLiterals:
    ...
    async def test_scope_section_accepted_with_section_param(
        self, mock_arq_pool: MagicMock
    ) -> None:
    ...
    async def test_scope_field_accepted_with_field_param(
        self, mock_arq_pool: MagicMock
    ) -> None:
...
class TestValidationErrors:
    ...
    async def test_scope_section_without_section_param_is_error(
        self, mock_arq_pool: MagicMock
    ) -> None:
    ...
    async def test_scope_field_without_field_param_is_error(
        self, mock_arq_pool: MagicMock
    ) -> None:
    ...
    async def test_offer_module_without_entity_or_offer_id_fails(
        self, mock_arq_pool: MagicMock
    ) -> None:
...
class TestToolRegistration:
    ...
    def test_extract_from_doc_added_to_extraction_tools(self) -> None:
        ...
        assert "extract_from_doc" in names, (
            "extract_from_doc must be registered in EXTRACTION_TOOLS"
        )
