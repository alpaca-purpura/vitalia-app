"""Tests for the ``web_research`` shared tool.

Previously lived under ``tools/interview/``; now in ``shared_tools`` so it's
bindable from any route (chat mode and guided mode).
"""
...
async def test_web_research_default_max_results() -> None:
    with patch(
        ...
        mock_service.search.assert_called_once_with(query="test", max_results=5)
