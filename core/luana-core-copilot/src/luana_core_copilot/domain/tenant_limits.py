...
_MEDIA_BYTES_MAX: int = (
    100 * 1024 * 1024
)  # 100 MiB (SaaS standard for microempresarios)
...
class CopilotTenantLimits:
    ...
    def __post_init__(self) -> None:
        ...
        if self.voice_rpm_override is not None:
            if not isinstance(self.voice_rpm_override, int) or isinstance(
                self.voice_rpm_override, bool
            ):
        ...
        if self.media_max_bytes_override is not None:
            if not isinstance(self.media_max_bytes_override, int) or isinstance(
                self.media_max_bytes_override, bool
            ):
