...
function recortar(s, max = 400) {
    const limpio = s.replace(/\s+/g, ' ').trim();
    return limpio.length > max ? limpio.slice(0, max) + '…' : limpio;
}
function anotar(sesion, tipo, detalle) {
    const ev = { ts: new Date().toISOString(), tipo, detalle };
...
async function correrSesion(sesion, prompt) {
...
const server = http.createServer((req, res) => {
    ...
    if (req.method === 'GET' && m) {
        ...
        if (!s)
            return json(res, 404, { error: 'sesión desconocida' });
