...
> **Slim stub (context-rot pass 2026-05-30 · materializada single-brand 2026-07-31).** Detalle operativo completo (4 steps verbatim con grep/run commands, ejemplos CORRECTO/INCORRECTO commit body, enforcement layers 7, penalizaciones) en `docs/rules-detail/anti-default-flip-audit.md` — load on-demand. **Origen:** failed `/pase-produccion` 2026-05-04 (commit `64738354` flipeó `USE_OUTBOX_PATTERN_*` False→True sin auditar → 25 BE failures + ~3h + ~500k tokens).
...
- Step 3 run commands (engine + vitalia)
