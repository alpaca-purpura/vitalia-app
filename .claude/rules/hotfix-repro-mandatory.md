...
La forma B (trace) vale cuando el repro local no es viable; el diagnóstico DEBE quedar anclado a evidencia concreta, nunca al texto del handoff. (Las `source` reales por surface: seam `live_verify_infra.observability_evidence` en `project.config.yaml`.)
...
- Commands verbatim de reproducción (vitalia vs engine)
