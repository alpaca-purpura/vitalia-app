...
## Test pre-prod
Clone DB workflow — runbook `docs/domains/migrations.md` (steps verbatim + troubleshooting). Resumen:
1. Crear `migration_test` DB (`createdb migration_test`)
2. `pg_dump --schema-only $PROD_DB | psql migration_test`
3. `alembic -c vitalia/backend/alembic.ini stamp <current_prod_rev>`
4. `${WS}/.venv/bin/alembic -c vitalia/backend/alembic.ini upgrade head`
