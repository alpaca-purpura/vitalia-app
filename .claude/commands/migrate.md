...
Steps:
1. Generate migration: `docker exec -t luana-dev-{brand}_backend_dev-1 alembic revision --autogenerate -m "$ARGUMENTS"`
...
4. Apply migration: `docker exec -t luana-dev-{brand}_backend_dev-1 alembic upgrade head`
5. Verify: `docker exec -t luana-dev-{brand}_backend_dev-1 alembic current`
