...
| `expect(isAuthed).toBe(true)` failed in setup | SPA didn't hydrate Clerk in 30s after sign-in | Check FE container logs for hydration errors. Possibly the dev server is OOM. `docker logs luana-dev-{brand}_frontend_dev-1 --tail 100` |
