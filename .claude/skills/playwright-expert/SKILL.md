...
1. **NATIVE Linux only (host).** `make e2e`, `make e2e-smoke`, `docker compose --profile e2e` are forbidden locally — they crash Linux. CI uses Docker, that is fine.
...
9. **`E2E_BASE_URL` is the canonical entry point.** If unset, Playwright spawns `next dev` — slow and flaky in Linux. Always export it locally.
