# Connections ManyChat Integration Map (engine `luana-core-connections`)
...
| File | Purpose |
|---|---|
| `vitalia/frontend/src/features/connections/` (⚠️ la vista manychat-view.tsx pre-reset NO existe hoy — rebuild pendiente) | `ManyChatView` component — two states: disconnected (API key input form) and connected (account info, test connection, disconnect). Uses Clerk auth, Shadcn UI, sonner toasts. |
| `vitalia/frontend/src/lib/api/connections.ts` (⚠️ FE pre-reset, no existe hoy) | API client functions: `connectionsApi.getManyChatStatus()`, `.connectManyChat()`, `.testManyChat()`, `.disconnectManyChat()`. Uses `fetchClient` with bearer token. |
