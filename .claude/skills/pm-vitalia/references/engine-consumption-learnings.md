# Engine Consumption — Learnings y Anti-patterns Catalogados

> Reference doc del skill `/pm-vitalia` (ex `pm-luana/references/brand-bootstrap-learnings.md`,
> migrado 2026-07-31 al fusionar los PMs en el repo standalone single-brand `vitalia-app`).
> **Lo VIGENTE acá:** la filosofía cardinal (engine-first: consumir, no reinventar) + el
> catálogo de anti-patterns AP1-AP7 — aplican cada vez que vitalia agrega un módulo nuevo.
> **Lo HISTÓRICO (read-only):** el patrón de bootstrap de marcas nuevas (Steps 0-8, checklist
> 13 pasos, tabla de puertos multi-brand) — en este repo NO se bootstrapean marcas nuevas
> (`_pm-sistema-template` eliminado); se conserva como contexto de origen de los anti-patterns.
> Referencias internas a `docs/promotion-protocol/` / `docs/portfolio/` / otras marcas son
> arqueológicas (archivadas o viven en el repo luana-platform).
...
- [ ] Step 11: (histórico multimarca) INFRA-MATRIX — retirado en repo standalone
